...
int ServProc(int argc, char *argv[])
{
    ...
    IScheduledHelper::SetInstance(&scheduledHelper);
    ...
    MapServer::GetInstance()->Start();

    LOG_NORMAL("map server stopped");
    LOG_UNINIT();
...
{
    auto result = WinService::GetInstance().RegCtrlHandler("MapServer", ServiceCtrlHandler, nullptr);
    if (result != ERROR_SUCCESS) return;

    result = WinService::GetInstance().SetCurrentState(SERVICE_START_PENDING, 5000);
    if (result != ERROR_SUCCESS) return;
