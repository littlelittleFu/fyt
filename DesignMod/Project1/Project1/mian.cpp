...
int main()
{
	...
	/***
	* 1、
	* 简单工厂 封装一个创建实例化过程的类 工厂方法 对于不同系列创建产品抽象出父类 在子类中创建各类产品
	* 策略模式   一个产品某个功能有不同的算法实现 添加成员算法抽象父类 构建时根据传入参数在实例化后使用过程中使用不同算法
	***/
	...
	/***
	* 2、装饰模式    在已有的功能类中 增加新的修饰 只在特定场景使用 将原有的功能类封装在新的修饰类中 在特定场景下 使用特定的修饰类 完成原有及新修饰的功能
	***/
